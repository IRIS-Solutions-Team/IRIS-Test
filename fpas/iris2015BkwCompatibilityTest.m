...
% Run plain vanilla kalman filter first
[~, kfdb] = filter( ...
    mInput,db_obs.tunedb,startHist:endHist,...
    'relative',false ...,'std',db_obs.dbstd...
    );


% running estimation
% warnStruct = warning('off','IrisToolboxDbase:NameNotExist');
[p_est,~,~,~,mod_est] = estimate( ...
    mInput,db_obs.tunedb,DateWrapper(startHist:endHist),priors,[],...
    'initVal','model',...
    'noSolution','penalty',...
    'filter',{'relative',false,'override',db_obs.dbstd, 'unitRootInitials', 'fixedUnknown'},...
    'summary', 'struct',...
    'solver',optimizer ...
    );
% warning(warnStruct.state,'IrisToolboxDbase:NameNotExist');
...
% Call fminsarch explicitly
[p_est1,~,~,~,mod_est1] = estimate(mInput,db_obs.tunedb,DateWrapper(startHist:endHist),priors,[],...
    'initVal','model',...
    'noSolution','penalty',...
    'filter',{'relative',false,'override',db_obs.dbstd, 'unitRootInitials', 'fixedUnknown'},...
    'summary', 'struct',...
    'solver',{'fminsearch', 'TolFun', 1e-1, 'MaxFunEvals', 150000, 'MaxIter', 15000});
