...
%% Test Scalar Series with Numeric Exogenous

exogenous = 0;
init = rand(order, 1);
x = Series(start-order:start-1, init);
...
%% Test Multivariate Series with Numeric Exogenous
...
%% Test Multivariate Series with Exogenous Series

init = rand(order, 3, 2);
exogenous = Series(start, rand(numOfPeriods, 3, 2));
x = Series(start-order:start-1, init);
actual = arf(x, [1, -1*ar], exogenous, start:finish);
