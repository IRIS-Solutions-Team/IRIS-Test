...
m1 = solve(m1);
m1 = sstate(m1);
assertEqual(testCase, real(m1.rhox), real(m1.rhoy));
assertEqual(testCase, real(m1.mux), real(m1.muy));
assertEqual(testCase, real(m1.x), real(m1.y));
...
m1 = solve(m1);
m1 = sstate(m1);
assertNotEqual(testCase, real(m1.rhox), real(m1.rhoy));
assertNotEqual(testCase, real(m1.mux), real(m1.muy));
assertNotEqual(testCase, real(m1.x), real(m1.y));
...
assertEqual(testCase, actStatus, expStatus);
...
if isOptim
    ...
    assertEqual(testCase, round(est0.rhoy, 1), m0.rhoy);
    assertNotEqual(testCase, est0.mux, m0.mux);
    assertNotEqual(testCase, est0.muy, m0.muy);
    assertNotEqual(testCase, est0.rhox, m0.rhox);
    assertNotEqual(testCase, est0.rhoy, m0.rhoy);
end
...
m = refresh(m);
assertEqual(testCase, m.std_epsy, [10, 20, 100]);
assertEqual(testCase, m.std_epsx, [10, 20, 100]*2);
assertEqual(testCase, m.corr_epsx__epsy, [10, 20, 100]*0.1);
assertEqual(testCase, m.corr_epsy__epsx, [10, 20, 100]*0.1);
