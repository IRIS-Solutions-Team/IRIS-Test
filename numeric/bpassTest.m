...
function testNoDetrend(this)
    ...
    data1 = series.bpass(data, [2, 40]);
    x1 = bpass(x, [2, 40]);
    data2 = series.bpass(data, [2, 40], 'Detrend', false);
...
function testAddTrend(this)
    data = this.TestData.Numeric;
    x = this.TestData.Series;
    data1 = series.bpass(data, [2, 40], 'AddTrend', true);
...
function testLevelTrend(this)
    ...
    data1 = series.bpass(data, [2, 40]);
    x1 = bpass(x, [2, 40]);
    [data2, trendData2] = series.bpass(data, [2, 40], 'Detrend', {'Diff', false, 'Connect', false});
